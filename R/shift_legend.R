...
shift_legend <- function(p){
    ...
    # check if p is a valid object (can remove if cowplot checks?) ----
    ...
    # Get empty facets ----
    empty.facet.panels <- cowplot::plot_to_gtable(p) %>%
    ...
    # But it turns out a bit too large
    # check for unfilled facet panels (original) ----
    #facet.panels <- grep("^panel", gp[["layout"]][["name"]])
    #empty.facet.panels <- sapply(facet.panels, function(i)
    # "zeroGrob" %in% class(gp[["grobs"]][[i]]))
    #empty.facet.panels <- facet.panels[empty.facet.panels]

    # If no empty facets, return plot ----
